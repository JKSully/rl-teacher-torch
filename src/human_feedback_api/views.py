...
ExperimentResource = namedtuple("ExperimentResource", [
                                'name', 'num_responses', 'started_at', 'pretty_time_elapsed'])
...
def _build_experiment_resource(experiment_name):
    comparisons = Comparison.objects.filter(
        experiment_name=experiment_name, responded_at__isnull=False)
    try:
...
def _all_comparisons(experiment_name, use_locking=True):
    ...
    not_in_progress = Q(shown_to_tasker_at__isnull=True) | Q(
        shown_to_tasker_at__lte=cutoff_time)
    finished_uploading_media = Q(created_at__lte=timezone.now(
    ) - timedelta(seconds=2))  # Give time for upload
...
def index(request):
    return render(request, 'index.html', context={
        'experiments': [experiment_name for experiment_name in
...
def list_comparisons(request, experiment_name):
    comparisons = Comparison.objects.filter(
        experiment_name=experiment_name).order_by('responded_at', '-priority')
...
def ajax_response(request, experiment_name):
    ...
    if debug:
        print("Answered comparison {} with {}".format(
            comparison_id, comparison.response))
